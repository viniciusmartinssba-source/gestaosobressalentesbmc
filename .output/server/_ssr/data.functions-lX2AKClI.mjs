...
//#region node_modules/.nitro/vite/services/ssr/assets/data.functions-lX2AKClI.js
...
var getHistory = createServerFn({ method: "GET" }).handler(getHistory_createServerFn_handler, async () => {
	const { supabaseAdmin } = await import("./client.server-KzwUIAkW.mjs");
	const { data, error } = await supabaseAdmin.from("movimentacoes").select(`
      *,
      profiles (nome),
      parques (nome),
      pecas (sap, descricao)
    `).order("data", { ascending: false }).limit(1e3);
	if (error) throw error;
	return (data ?? []).map((m) => ({
		id: m.id,
		dataISO: m.data || (/* @__PURE__ */ new Date()).toISOString(),
